...
!!! note
 ...
 The PostGIS module is an extension to the PostgreSQL backend server. As such, PostGIS 3.6.4 *requires* full PostgreSQL server headers access in order to compile. It can be built against PostgreSQL versions 12 - 18. Earlier versions of PostgreSQL are *not* supported.
...
## Getting the Source
 ...
 Retrieve the PostGIS source archive from the downloads website [https://download.osgeo.org/postgis/source/postgis-3.6.4.tar.gz](https://download.osgeo.org/postgis/source/postgis-3.6.4.tar.gz)


```
wget https://download.osgeo.org/postgis/source/postgis-3.6.4.tar.gz
tar -xvzf postgis-3.6.4.tar.gz
cd postgis-3.6.4
```


 This will create a directory called `postgis-3.6.4` in the current working directory.

...
!!! note
 ...
 The PostGIS module is an extension to the PostgreSQL backend server. As such, PostGIS 3.6.3 *requires* full PostgreSQL server headers access in order to compile. It can be built against PostgreSQL versions 12 - 18. Earlier versions of PostgreSQL are *not* supported.
...
## Getting the Source
 ...
 Retrieve the PostGIS source archive from the downloads website [https://download.osgeo.org/postgis/source/postgis-3.6.3.tar.gz](https://download.osgeo.org/postgis/source/postgis-3.6.3.tar.gz)
...
```
 ...
 This will create a directory called `postgis-3.6.3` in the current working directory.

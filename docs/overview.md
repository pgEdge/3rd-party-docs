...
The PostgreSQL YUM repository supports `pg_stat_monitor` for all [supported versions](overview.md#supported-versions) for the following platforms:
...
* [Percona repositories](overview.md#installing-from-percona-repositories)
* [PostgreSQL PGDG yum repositories](overview.md#installing-from-postgresql-yum-repositories)
* [PGXN](overview.md#installing-from-pgxn)
* [source code](overview.md#building-from-source)
...
>
>⚠️ For PostgreSQL 13 `pg_stat_monitor` **must** follow `pg_stat_statements`. For example, `ALTER SYSTEM SET shared_preload_libraries = 'foo, pg_stat_statements, pg_stat_monitor'`.
>

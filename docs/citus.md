...
1. [Citus](https://github.com/citusdata/citus) database extension to
...
4. [REST API](patroni-configuration/yaml_configuration.md#restapi_settings) access should be allowed from worker
...
1. Patroni will set `bootstrap.dcs.synchronous_mode` to [quorum](replication_modes.md#quorum_mode)
    if it is not explicitly set to any other value.

2. `citus` extension will be automatically added to `shared_preload_libraries`.

3. If `max_prepared_transactions` isn't explicitly set in the global
    [dynamic configuration](patroni-configuration/dynamic_configuration.md#dynamic_configuration) Patroni will
    automatically set it to `2*max_connections`.

4. The `citus.local_hostname` GUC value will be adjusted from `localhost` to the
...
6. Current superuser [credentials](patroni-configuration/yaml_configuration.md#postgresql_settings) will be added to the `pg_dist_authinfo`
...
7. The coordinator primary node will automatically discover worker primary
    nodes and add them to the `pg_dist_node` table using the
    `citus_add_node()` function.

8. Patroni will also maintain `pg_dist_node` in case failover/switchover
...
It results in two major differences in [patronictl](patronictl.md#patronictl) behaviour when `patroni.yaml` has the `citus` section comparing with the usual:
...
2. For all `patronictl` commands the new option is introduced, named
    ...
    taken from the `patroni.yaml`. For example, [patronictl pause](patronictl.md#patronictl_pause) will
    enable the maintenance mode by default for the `group` that is set in the
    `citus` section, but for example for [patronictl switchover](patronictl.md#patronictl_switchover) or
    [patronictl remove](patronictl.md#patronictl_remove) the group must be explicitly specified.

An example of [patronictl list](patronictl.md#patronictl_list) output for the Citus cluster:
...
An example of [patronictl switchover](patronictl.md#patronictl_switchover) on the worker cluster:
...
Starting from Patroni v4.0.0 Citus secondary nodes without `noloadbalance` [tag](patroni-configuration/yaml_configuration.md#tags_settings) are also registered in `pg_dist_node`. However, to use secondary nodes for read-only queries applications need to change [citus.use_secondary_nodes](https://docs.citusdata.com/en/latest/develop/api_guc.html#citus-use-secondary-nodes-enum) GUC.
...
All Kubernetes objects are discovered by Patroni using the `label selector`__, therefore all Pods with Patroni&Citus and Endpoints/ConfigMaps must have similar labels, and Patroni must be configured to use them using Kubernetes [settings](patroni-configuration/yaml_configuration.md#kubernetes_settings) or [environment variables](patroni-configuration/ENVIRONMENT.md#kubernetes_environment).
...
You can find a complete example of Patroni deployment on Kubernetes with Citus support in the `kubernetes`__ folder of the Patroni repository.
...
First, please read about upgrading Citus version in the `documentation`__. There is one minor change in the process. When executing upgrade, you have to use [patronictl restart](patronictl.md#patronictl_restart) instead of `systemctl restart` to restart PostgreSQL.

__ https://docs.citusdata.com/en/latest/admin_guide/upgrading_citus.html

The PostgreSQL major upgrade with Citus is a bit more complex. You will have to combine techniques used in the Citus documentation about major upgrades and Patroni documentation about [PostgreSQL major upgrade](existing_data.md#major_upgrade). Please keep in mind that Citus cluster consists of many Patroni clusters (coordinator and workers) and they all have to be upgraded independently.

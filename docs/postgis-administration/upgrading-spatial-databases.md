...
```sql
ALTER EXTENSION postgis UPDATE TO "dev";
ALTER EXTENSION postgis_topology UPDATE TO "dev";
```
...
```
No migration path defined for … to dev
```
...
```
Version "dev" of extension "postgis" is already installed
```
...
```sql
ALTER EXTENSION postgis UPDATE TO "devnext";
ALTER EXTENSION postgis_topology UPDATE TO "devnext";
```
...
```
ls `pg_config --sharedir`/contrib/postgis-dev/*_upgrade.sql
```

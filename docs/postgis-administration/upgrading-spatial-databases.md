...
```sql
ALTER EXTENSION postgis UPDATE TO "3.6.2";
ALTER EXTENSION postgis_topology UPDATE TO "3.6.2";
```
...
```
No migration path defined for … to 3.6.2
```
...
```
Version "3.6.2" of extension "postgis" is already installed
```
...
```sql
ALTER EXTENSION postgis UPDATE TO "3.6.2next";
ALTER EXTENSION postgis_topology UPDATE TO "3.6.2next";
```
...
```
ls `pg_config --sharedir`/contrib/postgis-3.6.2/*_upgrade.sql
```

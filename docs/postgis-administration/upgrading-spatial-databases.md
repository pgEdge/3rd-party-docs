...
```sql
ALTER EXTENSION postgis UPDATE TO "3.5.7";
ALTER EXTENSION postgis_topology UPDATE TO "3.5.7";
```
...
```
No migration path defined for … to 3.5.7
```
...
```
Version "3.5.7" of extension "postgis" is already installed
```
...
```sql
ALTER EXTENSION postgis UPDATE TO "3.5.7next";
ALTER EXTENSION postgis_topology UPDATE TO "3.5.7next";
```
...
```
ls `pg_config --sharedir`/contrib/postgis-3.5.7/*_upgrade.sql
```

...
ALTER EXTENSION postgis_topology UPDATE TO "3.6.4";
```
...
```
No migration path defined for … to 3.6.4
```
...
```
Version "3.6.4" of extension "postgis" is already installed
```
...
```sql
ALTER EXTENSION postgis UPDATE TO "3.6.4next";
ALTER EXTENSION postgis_topology UPDATE TO "3.6.4next";
```
...
```
ls `pg_config --sharedir`/contrib/postgis-3.6.4/*_upgrade.sql
```

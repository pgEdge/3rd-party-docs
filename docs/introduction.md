...
Patroni is in active development and accepts contributions. See our [Contributing](CONTRIBUTING.md#contributing) section below for more details.

We report new releases information [here](releases.md#releases).

## Technical Requirements/Installation

Go [here](installation.md#installation) for guidance on installing and upgrading Patroni on various platforms.
...
Go [here](patroni-configuration/yaml_configuration.md#yaml_configuration) for comprehensive information about settings for etcd, consul, and ZooKeeper. And for an example, see [postgres0.yml](https://github.com/patroni/patroni/blob/master/postgres0.yml).

## Environment Configuration

Go [here](patroni-configuration/ENVIRONMENT.md#environment) for comprehensive information about configuring(overriding) settings via environment variables.

## Replication Choices

Patroni uses Postgres' streaming replication, which is asynchronous by default. Patroni's asynchronous replication configuration allows for `maximum_lag_on_failover` settings. This setting ensures failover will not occur if a follower is more than a certain number of bytes behind the leader. This setting should be increased or decreased based on business requirements. It's also possible to use synchronous replication for better durability guarantees. See [replication modes documentation](replication_modes.md#replication_modes) for details.

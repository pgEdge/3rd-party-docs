...
- `GET /standby-leader`: returns HTTP status code **200** only when the Patroni node is running as the leader in a [standby cluster](standby_cluster.md#standby_cluster).
...
**Example:** An unlocked cluster with [DCS failsafe mode](dcs_failsafe_mode.md#dcs_failsafe_mode) enabled
...
**Example:** A cluster with the [pause mode](pause.md#pause) enabled
...
$ curl -s http://localhost:8008/config | jq . { "ttl": 30, "loop_wait": 10, "retry_timeout": 10, "maximum_lag_on_failover": 1048576, "postgresql": { "use_slots": true, "use_pg_rewind": true, "parameters": { "hot_standby": "on", "wal_level": "hot_standby", "max_wal_senders": 5, "max_replication_slots": 5, "max_connections": "100" } } }
...
$ curl -s http://localhost:8008/patroni | jq . { "database_system_identifier": "6287881213849985952", "postmaster_start_time": "2024-08-28 19:39:26.352526+00:00", "xlog": { "location": 2197818976 }, "timeline": 1, "dcs_last_seen": 1724874545, "database_system_identifier": "7408277255830290455", "pending_restart": true, "pending_restart_reason": { "max_connections": { "old_value": "100", "new_value": "101" } }, "patroni": { "version": "4.0.0", "scope": "batman", "name": "patroni1" }, "state": "running", "role": "primary", "server_version": 160004 }
...
$ curl -s -XPATCH -d  '{"postgresql":{"parameters":{"max_connections":null}}}'  http://localhost:8008/config | jq . { "ttl": 20, "loop_wait": 5, "retry_timeout": 10, "maximum_lag_on_failover": 1048576, "postgresql": { "use_slots": true, "use_pg_rewind": true, "parameters": { "hot_standby": "on", "unix_socket_directories": ".", "wal_level": "hot_standby", "max_wal_senders": 5, "max_replication_slots": 5 } } }
...
$ curl -s http://localhost:8008/switchover -XPOST -d '{"leader":"postgresql1"}' Successfully switched over to "postgresql2"
...
$ curl -s http://localhost:8008/switchover -XPOST -d  '{"leader":"postgresql1","candidate":"postgresql2"}' Successfully switched over to "postgresql2"
...
$ curl -s http://localhost:8008/failover -XPOST -d '{"candidate":"postgresql1"}' Successfully failed over to "postgresql1"

!!! warning


[Be very careful](#failover_healthcheck) when using this endpoint, as this can cause data loss in certain situations. In most cases, [the switchover endpoint](#switchover_api) satisfies the administrator's needs.

`POST /switchover` and `POST /failover` endpoints are used by [patronictl switchover](patronictl.md#patronictl_switchover) and [patronictl failover](patronictl.md#patronictl_failover), respectively.

`DELETE /switchover` is used by [patronictl flush cluster-name switchover](patronictl.md#patronictl_flush_parameters).
...
- in case of a switchover in a healthy cluster or an automatic failover, not exceed maximum replication lag (`maximum_lag_on_failover` [configuration parameter](patroni-configuration/dynamic_configuration.md#dynamic_configuration));

- in case of a switchover in a healthy cluster or an automatic failover, not have a timeline number smaller than the cluster timeline if `check_timeline` [configuration parameter](patroni-configuration/dynamic_configuration.md#dynamic_configuration) is set to `true`;

- in [synchronous mode](replication_modes.md#synchronous_mode):
...
- it is not in the `/sync` key members when synchronous mode is enabled; - its lag exceeds the maximum replication lag allowed; - it has the timeline number smaller than the last known cluster timeline.
...
`POST /restart` and `DELETE /restart` endpoints are used by [patronictl restart](patronictl.md#patronictl_restart) and [patronictl flush cluster-name restart](patronictl.md#patronictl_flush_parameters) respectively.
<a id="reload_endpoint"></a>

## Reload endpoint

The `POST /reload` call will order Patroni to re-read and apply the configuration file. This is the equivalent of sending the `SIGHUP` signal to the Patroni process. In case you changed some of the Postgres parameters which require a restart (like **shared_buffers**), you still have to explicitly do the restart of Postgres by either calling the `POST /restart` endpoint or with the help of [patronictl restart](patronictl.md#patronictl_restart).

The reload endpoint is used by [patronictl reload](patronictl.md#patronictl_reload).

## Reinitialize endpoint

`POST /reinitialize`: reinitialize the PostgreSQL data directory on the specified node. It is allowed to be executed only on replicas. Once called, it will remove the data directory and start `pg_basebackup` or some alternative [replica creation method](replica_bootstrap.md#custom_replica_creation).
...
The reinitialize endpoint is used by [patronictl reinit](patronictl.md#patronictl_reinit).

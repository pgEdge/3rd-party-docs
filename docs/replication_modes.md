...
Synchronous mode can be switched on and off using `patronictl edit-config` command or via Patroni REST interface. See [dynamic configuration](patroni-configuration/dynamic_configuration.md#dynamic_configuration) for instructions.
...
The quorum-based synchronous mode could be enabled by setting `synchronous_mode` to `quorum` using `patronictl edit-config` command or via Patroni REST interface. See [dynamic configuration](patroni-configuration/dynamic_configuration.md#dynamic_configuration) for instructions.

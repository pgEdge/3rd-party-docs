...
The high availability of a PostgreSQL cluster deployed in multiple data centers is based on replication, which can be synchronous or asynchronous (see [replication modes](replication_modes.md#replication_modes)).
...
Regarding postgres, we must deploy at least 2 nodes, in different DC. Then you have to set `synchronous_mode: true` in the global [dynamic configuration](patroni-configuration/dynamic_configuration.md#dynamic_configuration).

This enables sync replication and the primary node will choose one of the nodes as synchronous.

## Asynchronous Replication

With only two data centers it would be better to have two independent etcd clusters and run Patroni [standby cluster](standby_cluster.md#standby_cluster) in the second data center. If the first site is down, you can MANUALLY promote the `standby_cluster`.
...
You should not use `pg_ctl promote` in this scenario, you need "manually promote" the healthy cluster by removing `standby_cluster` section from the [dynamic configuration](patroni-configuration/dynamic_configuration.md#dynamic_configuration).

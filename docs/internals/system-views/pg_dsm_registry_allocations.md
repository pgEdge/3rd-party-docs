...
## `pg_dsm_registry_allocations`
 ...
 The `pg_dsm_registry_allocations` view shows shared memory allocations tracked in the dynamic shared memory (DSM) registry. This includes memory allocated by extensions using the mechanisms detailed in [Allocating Dynamic Shared Memory after Startup](../../server-programming/extending-sql/c-language-functions.md#xfunc-shared-addin-dynamic).

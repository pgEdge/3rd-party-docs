...
```
 ...
 The form `GROUP BY ALL` with no explicit *grouping_elements* provided is equivalent to writing `GROUP BY` with the numbers of all `SELECT` output columns that do not contain either an aggregate function referring to the same query level or a window function.

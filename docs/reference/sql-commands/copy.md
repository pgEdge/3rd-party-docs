...
## Notes
 ...
 `COPY TO` can be used with plain tables and populated materialized views. For example, <code>COPY </code><em>table</em><code>
    TO</code> copies the same rows as <code>SELECT * FROM ONLY </code><em>table</em>. However it doesn't directly support other relation types, such as partitioned tables, inheritance child tables, or views. To copy all rows from such relations, use <code>COPY (SELECT * FROM
...
## File Formats { #sql-copy-file-formats }


### Text Format { #sql-copy-text-format }
...
### CSV Format { #sql-copy-csv-format }
...
### Binary Format { #sql-copy-binary-format }

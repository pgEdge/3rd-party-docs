...
## Compatibility
 ...
 The SQL standard suggests, without saying so in so many words, that the subcommands in `CREATE SCHEMA` can appear in any order; but the only actual requirement it gives is that foreign keys referencing table definitions that appear in later `CREATE SCHEMA` subcommands should be accepted. The present PostgreSQL implementation handles that case, but does not handle all other cases of forward references in subcommands; it might sometimes be necessary to reorder the subcommands in order to avoid forward references.

...
| `Buffer` | The server process is waiting for access to a data buffer. See [wait-event-buffer-table](#wait-event-buffer-table). |
...
### `pg_stat_recovery` { #monitoring-pg-stat-recovery-view }

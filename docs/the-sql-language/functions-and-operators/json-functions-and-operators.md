...
<td>Constructs a JSON array from either a series of <em>value_expression</em> parameters or from the results of <em>query_expression</em>, which must be a SELECT query returning a single column. If <code>ABSENT ON NULL</code> is specified, NULL values are ignored. This is always the case if a <em>query_expression</em> is used. If the query returns no rows, an empty JSON array is returned.<br><code>json_array(1,true,json '{"a":null}')</code> <code>[1, true, {"a":null}]</code><br><code>json_array(SELECT * FROM (VALUES(1),(2)) t)</code> <code>[1, 2]</code></td>
</tr>
<tr>

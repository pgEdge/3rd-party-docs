...
-  Avoid integer overflow in allocation-size calculations within libpq (Jacob Champion) [&sect;](https://postgr.es/c/7eb8fcad8)
...
-  Prevent “unrecognized node type” errors when a SQL/JSON function such as `JSON_VALUE` has a `DEFAULT` clause containing a `COLLATE` expression (Jian He) [&sect;](https://postgr.es/c/dc9125111) [&sect;](https://postgr.es/c/1baae827e)
-  Avoid incorrect optimization of variable-free `HAVING` clauses with grouping sets (Richard Guo) [&sect;](https://postgr.es/c/40c242830) [&sect;](https://postgr.es/c/ee49f2cf4)
-  Do not use parallelism in hash right semi joins (Richard Guo) [&sect;](https://postgr.es/c/ef6168baf)

   The case does not work reliably due to a race condition in updating the join's shared hash table.
-  Avoid possible division-by-zero when creating ordered-append plans (Richard Guo) [&sect;](https://postgr.es/c/500f64636)

   This mistake could result in incorrect selection of the cheapest path, or in an assertion failure in debug builds.
-  Fix planner failure with index types that can do ordered access but not index-only scans (Maxime Schoemans) [&sect;](https://postgr.es/c/74197bdc8)
...
-  Remove faulty assertion in btree index cleanup (Peter Geoghegan) [&sect;](https://postgr.es/c/61de81a49)
-  Avoid possible out-of-memory or “invalid memory alloc request size” failures during parallel GIN index build (Tomas Vondra) [&sect;](https://postgr.es/c/a26b753a0)
-  Ensure that BRIN autosummarization provides a snapshot for index expressions that need one (Álvaro Herrera) [&sect;](https://postgr.es/c/419ffde23) [&sect;](https://postgr.es/c/8733f0b54)
...
-  Fix integer-overflow hazard in BRIN index scans when the table contains close to 2<sup>32</sup> pages (Sunil S) [&sect;](https://postgr.es/c/715983a81)

   This oversight could result in an infinite loop or scanning of unneeded table pages.
-  Fix incorrect zero-extension of stored values in JIT-generated tuple deforming code (David Rowley) [&sect;](https://postgr.es/c/ceb51d09b)
...
-  Fix rare crash when processing hashed `GROUPING SETS` queries (David Rowley) [&sect;](https://postgr.es/c/0b6a02f03)
-  Repair faulty hash-table-size-choosing logic in hash joins (Tomas Vondra) [&sect;](https://postgr.es/c/aa151022e)

   Hash joins sometimes used more memory than intended, or failed to divide it in an efficient way.
-  Improve relation lookup logic in statistics manipulation functions (Nathan Bossart) [&sect;](https://postgr.es/c/c8af5019b) [&sect;](https://postgr.es/c/15d7dded0)
...
-  Fix incorrect logic for caching result-relation information for triggers (David Rowley, Amit Langote) [&sect;](https://postgr.es/c/a2387c32f)
...
-  Fix crash during EvalPlanQual rechecks on partitioned tables (David Rowley, Amit Langote) [&sect;](https://postgr.es/c/1296dcf18)
-  Fix EvalPlanQual handling of foreign or custom joins that do not have an alternative local-join plan prepared for EPQ (Masahiko Sawada, Etsuro Fujita) [&sect;](https://postgr.es/c/b14144325)
...
-  Avoid duplicating hash partition constraints during `DETACH CONCURRENTLY` (Haiyang Li) [&sect;](https://postgr.es/c/08c037dff)
...
-  Disallow generated columns in partition keys (Jian He, Ashutosh Bapat) [&sect;](https://postgr.es/c/ba99c9491)
...
-  Disallow generated columns in `COPY ... FROM ... WHERE` clauses (Peter Eisentraut, Jian He) [&sect;](https://postgr.es/c/0f9e0068b)
...
-  Prevent setting a column as identity if it has a not-null constraint but the constraint is marked as invalid (Jian He) [&sect;](https://postgr.es/c/d9ffc2729)

   Identity columns must be not-null, but the check for that missed this edge case.
-  Avoid potential use-after-free in parallel vacuum (Kevin Oommen Anish) [&sect;](https://postgr.es/c/76613b539)

   This bug seems to have no consequences in standard builds, but it's theoretically a hazard.
-  Fix visibility checking for statistics objects in `pg_temp` (Noah Misch) [&sect;](https://postgr.es/c/d024160ff)
...
-  Fix minor memory leak during WAL replay of database creation (Nathan Bossart) [&sect;](https://postgr.es/c/33e7b4a7c)
-  Fix incorrect reporting of replication lag in `pg_stat_replication` view (Fujii Masao) [&sect;](https://postgr.es/c/9670032cc)

   If any standby server's replay LSN stopped advancing, the `write_lag` and `flush_lag` columns would eventually stop updating.
-  Avoid duplicative log messages about invalid `primary_slot_name` settings (Fujii Masao) [&sect;](https://postgr.es/c/6ff7ba9fe)
-  Avoid failures when `synchronized_standby_slots` references nonexistent replication slots (Shlok Kyal) [&sect;](https://postgr.es/c/b45a8d7d8)
-  Remove the unfinished slot state file after failing to write a replication slot's state to disk (Michael Paquier) [&sect;](https://postgr.es/c/9a6ea00ac)
...
-  Fix mishandling of lock timeout signals in parallel apply workers for logical replication (Hayato Kuroda) [&sect;](https://postgr.es/c/37fc5de43)

   The same signal number was being used for both worker shutdown and lock timeout, leading to confusion.
-  Avoid unwanted WAL receiver shutdown when switching from streaming to archive WAL source (Xuneng Zhou) [&sect;](https://postgr.es/c/a14201073)
...
-  Fix use-after-free issue in the relation synchronization cache maintained by the pgoutput logical decoding plugin (Vignesh C, Masahiko Sawada) [&sect;](https://postgr.es/c/32b95fc71)
...
-  Avoid unnecessary invalidation of logical replication slots (Bertrand Drouvot) [&sect;](https://postgr.es/c/bf3dba508)
-  Re-establish special case for `C` collation in locale setup (Jeff Davis) [&sect;](https://postgr.es/c/3ebea75f9)
...
-  Fix incorrect printing of messages about failures in checking whether the user has Windows administrator privilege (Bryan Green) [&sect;](https://postgr.es/c/b48ae226e)
...
-  Avoid crash when attempting to test PostgreSQL with certain libsanitizer options (Emmanuel Sibi, Jacob Champion) [&sect;](https://postgr.es/c/6d8acb777)
-  Fix false memory-context-checking warnings in debug builds on 64-bit Windows (David Rowley) [&sect;](https://postgr.es/c/af3a79e08)
-  Correctly handle `GROUP BY DISTINCT` in PL/pgSQL assignment statements (Tom Lane) [&sect;](https://postgr.es/c/78a284b0b)
...
-  Avoid leaking memory when handling a SQL error within PL/Python (Tom Lane) [&sect;](https://postgr.es/c/447a794f6)

   This fixes a session-lifespan memory leak introduced in our previous minor releases.
-  Fix libpq's handling of socket-related errors on Windows within its GSSAPI logic (Ning Wu, Tom Lane) [&sect;](https://postgr.es/c/d83879a32)
...
-  Fix dumping of non-inherited not-null constraints on inherited table columns (Dilip Kumar) [&sect;](https://postgr.es/c/0fe07fa11)

   pg_dump failed to preserve such constraints when dumping from a pre-v18 server.
-  Fix pg_dump's sorting of foreign key constraints (Álvaro Herrera) [&sect;](https://postgr.es/c/162e70ea0)

   Ensure consistent ordering of these database objects, as was already done for other object types.
-  Fix assorted errors in the data compression logic in pg_dump and pg_restore (Daniel Gustafsson, Tom Lane) [&sect;](https://postgr.es/c/8980c724b) [&sect;](https://postgr.es/c/6a4009747) [&sect;](https://postgr.es/c/aa1fcd087)
...
-  Fix pgbench to error out cleanly if a `COPY` operation is started (Anthonin Bonnefoy) [&sect;](https://postgr.es/c/c00637b5f)

   pgbench doesn't intend to support this case, but previously it went into an infinite loop.
-  Fix pgbench's reporting of multiple errors (Yugo Nagata) [&sect;](https://postgr.es/c/29aabbc43)

   In cases where two successive `PQgetResult` calls both fail, pgbench might report the wrong error message.
-  In pgbench, fix faulty assertion about errors in pipeline mode (Yugo Nagata) [&sect;](https://postgr.es/c/c736808e0)
-  Fix per-file memory leakage in pg_combinebackup (Tom Lane) [&sect;](https://postgr.es/c/e2072b47b)
-  Ensure that `contrib/pg_buffercache` functions can be canceled (Satyanarayana Narlapuram, Yuhang Qiu) [&sect;](https://postgr.es/c/71aa2e114) [&sect;](https://postgr.es/c/0beb7e933)

   Some code paths were capable of running for a long time without checking for interrupts.
-  Fix `contrib/pg_prewarm`'s privilege checks for indexes (Ayush Vatsa, Nathan Bossart) [&sect;](https://postgr.es/c/3ccf8e9ac) [&sect;](https://postgr.es/c/c29d32d27)
...
-  In `contrib/pg_stat_statements`, avoid crash when two or more constants are marked as having the same location in the SQL statement text (Sami Imseih, Dmitry Dolgov) [&sect;](https://postgr.es/c/b1635c166)
-  Make `contrib/pgstattuple` more robust about empty or invalid index pages (Nitin Motiani) [&sect;](https://postgr.es/c/fc295beb7)
...
-  Harden our read and write barrier macros to satisfy Clang (Thomas Munro) [&sect;](https://postgr.es/c/f8ccab0e9)
...
-  Fix PGXS build infrastructure to support building NLS `po` files for extensions (Ryo Matsumura) [&sect;](https://postgr.es/c/6aa04a60c)

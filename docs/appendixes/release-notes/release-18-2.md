...
-  Harden selectivity estimators against being attached to operators that accept unexpected data types (Tom Lane) [&sect;](https://postgr.es/c/66ddac698) [&sect;](https://postgr.es/c/b69af3dda)
...
-  Fix buffer overrun in `contrib/pgcrypto`'s PGP decryption functions (Michael Paquier) [&sect;](https://postgr.es/c/209f387b8)
...
-  Fix inadequate validation of multibyte character lengths (Thomas Munro, Noah Misch) [&sect;](https://postgr.es/c/df0852fe0) [&sect;](https://postgr.es/c/efef05ba9) [&sect;](https://postgr.es/c/7b5fc85be) [&sect;](https://postgr.es/c/b0f5d25bc) [&sect;](https://postgr.es/c/b42709194) [&sect;](https://postgr.es/c/4543b02af)
...
-  Harden `contrib/pg_trgm` against changes in string lowercasing behavior (Heikki Linnakangas) [&sect;](https://postgr.es/c/e0965fb1a) [&sect;](https://postgr.es/c/18548681d)
...
-  Fix inconsistent case-insensitive matching in `contrib/ltree` (Jeff Davis) [&sect;](https://postgr.es/c/806555e30) [&sect;](https://postgr.es/c/8993bf099)
...
-  When using `ALTER TABLE ... ADD CONSTRAINT` to add a not-null constraint with an explicit name, if the column is already marked NOT NULL, require that the provided name match the existing constraint name (Álvaro Herrera, Srinath Reddy Sadipiralla) [&sect;](https://postgr.es/c/492a69e14)
-  Don't allow CTE references in sub-selects to determine semantic levels of aggregate functions (Tom Lane) [&sect;](https://postgr.es/c/12bc32917)
...
-  Fix trigger transition table capture for `MERGE` in CTE queries (Dean Rasheed) [&sect;](https://postgr.es/c/c6ce4dcf9)
...
-  Fix incorrect pruning of rowmarks belonging to non-relation rangetable entries, such as subqueries (Dean Rasheed) [&sect;](https://postgr.es/c/f335457e8)
...
-  Fix failure when all children of a partitioned target table of an update or delete have been pruned (Amit Langote) [&sect;](https://postgr.es/c/9f4b7bfc5)

   In such cases, the executor could report “could not find junk ctid column” errors, even though nothing needs to be done.
-  Fix expression evaluation bug for a sub-select within an array subscript (Andres Freund) [&sect;](https://postgr.es/c/bdc5dedfc)
-  Fix text substring search for non-deterministic collations (Laurenz Albe) [&sect;](https://postgr.es/c/18b349315)

   When using a non-deterministic collation, we failed to detect a match occurring at the very end of the searched string.
-  Avoid possible planner failure when a query contains duplicate window function calls (Meng Zhang, David Rowley) [&sect;](https://postgr.es/c/ccde5be68)
...
-  Fix planner error with set-returning functions and grouping sets (Richard Guo) [&sect;](https://postgr.es/c/382ce9cb7)
...
-  Avoid incorrect optimization when a subquery's grouping clause contains a volatile or set-returning function (Richard Guo) [&sect;](https://postgr.es/c/7650eabb6)
...
-  Look through PlaceHolderVar nodes when searching for statistics about an expression (Richard Guo) [&sect;](https://postgr.es/c/7e9f852a7)
...
-  Look through no-op PlaceHolderVar nodes when matching expressions to indexes (Richard Guo) [&sect;](https://postgr.es/c/b4cf74420)
...
-  Fix planner's conversion of `OR` clauses to ScalarArrayOp index conditions (Tender Wang, Tom Lane) [&sect;](https://postgr.es/c/bf5b13a8a)
...
-  Allow indexscans on partial hash indexes even when the index's predicate implies the truth of the WHERE clause (Tom Lane) [&sect;](https://postgr.es/c/a212877dc)
...
-  Do not emit WAL for unlogged BRIN indexes (Kirill Reshke) [&sect;](https://postgr.es/c/d77a5f981)
...
-  Use the correct ordering function in parallel GIN index builds (Tomas Vondra) [&sect;](https://postgr.es/c/eee71a66c)
...
-  Prevent truncation of CLOG that is still needed by unread `NOTIFY` messages (Joel Jacobson, Heikki Linnakangas) [&sect;](https://postgr.es/c/321ec5462) [&sect;](https://postgr.es/c/7b069a187) [&sect;](https://postgr.es/c/82fa6b78d)

   This fix prevents “could not access status of transaction” errors when a backend is slow to absorb `NOTIFY` messages.
-  Escalate errors occurring during `NOTIFY` message processing to FATAL, i.e. close the connection (Heikki Linnakangas) [&sect;](https://postgr.es/c/aab4a84bb)
...
-  Consider grouping expressions when computing a query ID hash (Jian He) [&sect;](https://postgr.es/c/9c3caad02)
...
-  Fix erroneous counting of updates in `EXPLAIN ANALYZE MERGE` with a concurrent update (Dean Rasheed) [&sect;](https://postgr.es/c/5749d95d4)
...
-  Fix bug in following update chain when locking a tuple (Jasper Smit) [&sect;](https://postgr.es/c/3e3a80f62)
...
-  Fix incorrect handling of incremental backups of large tables (Robert Haas, Oleg Tkachenko) [&sect;](https://postgr.es/c/c80b0c9d6)
...
-  Fix potential backend process crash at process exit due to trying to release a lock in an already-unmapped shared memory segment (Rahila Syed) [&sect;](https://postgr.es/c/1943ceb38)
-  Fix race condition in async I/O code (Andres Freund) [&sect;](https://postgr.es/c/7f1b3a4ce)

   It was possible for the result code of an asynchronous I/O operation to be overwritten before it was fetched.
-  Guard against incorrect truncation of the multixact log after a crash (Heikki Linnakangas) [&sect;](https://postgr.es/c/09532a78b)
-  Fix possibly mis-encoded result of `pg_stat_get_backend_activity()` (Chao Li) [&sect;](https://postgr.es/c/06907e864)
...
-  Guard against recursive memory context logging (Fujii Masao) [&sect;](https://postgr.es/c/b863d8d87)
...
-  Fix memory context usage when reinitializing a parallel execution context (Jakub Wartak, Jeevan Chalke) [&sect;](https://postgr.es/c/57df5ab80)
...
-  Set next multixid's offset when creating a new multixid, to remove the wait loop that was needed in corner cases (Andrey Borodin) [&sect;](https://postgr.es/c/e46041fd9) [&sect;](https://postgr.es/c/02ba5e3be)

   The previous logic could get stuck waiting for an update that would never occur.
-  Avoid rewriting data-modifying CTEs more than once (Bernice Southey, Dean Rasheed) [&sect;](https://postgr.es/c/b880d9a02)
...
-  Allow retrying initialization of a DSM registry entry (Nathan Bossart) [&sect;](https://postgr.es/c/b83bcc0df)
...
-  Avoid failure of NUMA status views when a page has been swapped out (Tomas Vondra) [&sect;](https://postgr.es/c/9796c4f56)
-  Avoid “operation not permitted” errors when querying NUMA page status with older libnuma versions (Tomas Vondra) [&sect;](https://postgr.es/c/482e98ac4)
-  Fail recovery if WAL does not exist back to the redo point indicated by the checkpoint record (Nitin Jadhav) [&sect;](https://postgr.es/c/68ebdf2b0)
...
-  Avoid scribbling on the source query tree during `ALTER PUBLICATION` (Sunil S) [&sect;](https://postgr.es/c/bea57a6b4)
...
-  Pass connection options specified in `CREATE SUBSCRIPTION ... CONNECTION` to the publisher's walsender (Fujii Masao) [&sect;](https://postgr.es/c/797fc5d1b)
...
-  Prevent invalidation of newly created or newly synced replication slots (Zhijie Hou) [&sect;](https://postgr.es/c/919c9fa13) [&sect;](https://postgr.es/c/1c60f7236) [&sect;](https://postgr.es/c/d3ceb2084)
...
-  Fix race condition in computing a replication slot's required xmin (Zhijie Hou) [&sect;](https://postgr.es/c/fd7c86cfa)

   This could lead to the error “cannot build an initial slot snapshot as oldest safe xid follows snapshot's xmin”.
-  During initial synchronization of a logical replication subscription, commit the addition of a `pg_replication_origin` entry before starting to copy data (Zhijie Hou) [&sect;](https://postgr.es/c/b07c32619)
...
-  Don't advance logical replication progress after a parallel worker apply failure (Zhijie Hou) [&sect;](https://postgr.es/c/2f7ffe124)

   The previous behavior allowed transactions to be lost by a subscriber.
-  Fix logical replication slotsync worker processes to handle LOCK_TIMEOUT signals correctly (Zhijie Hou) [&sect;](https://postgr.es/c/6c61c69d5)

   Previously, timeout signals were effectively ignored.
-  Fix possible failure with “unexpected data beyond EOF” during restart of a streaming replica server (Anthonin Bonnefoy) [&sect;](https://postgr.es/c/9ed411e08)
-  Fix error reporting for SQL/JSON path type mismatches (Jian He) [&sect;](https://postgr.es/c/15ba0702c)
...
-  Fix erroneous tracking of column position when parsing partition range bounds (myzhen) [&sect;](https://postgr.es/c/c35e5dd9a)
...
-  Fix assorted minor errors in error messages (Man Zeng, Tianchen Zhang) [&sect;](https://postgr.es/c/acfa422c3) [&sect;](https://postgr.es/c/2ca4464b6) [&sect;](https://postgr.es/c/ab61f0087) [&sect;](https://postgr.es/c/69ee81932) [&sect;](https://postgr.es/c/cff2ef984)
...
-  Fix failure to perform function inlining when doing JIT compilation with LLVM version 17 or later (Anthonin Bonnefoy) [&sect;](https://postgr.es/c/f1c6b153c)
-  Adjust our JIT code to work with LLVM 21 (Holger Hoffstätte) [&sect;](https://postgr.es/c/912cfa314)

   The previous coding failed to compile on aarch64 machines.
-  Fix aarch64-specific code to build with old (RHEL7-era) system header files (Tom Lane) [&sect;](https://postgr.es/c/db4eba152) [&sect;](https://postgr.es/c/6a5170755)
-  Fix incorrect configure probe for `io_uring_queue_init_mem()` (Masahiko Sawada) [&sect;](https://postgr.es/c/640772c6d)
...
-  Add new server parameter [file_extend_method](../../server-administration/server-configuration/resource-consumption.md#guc-file-extend-method) to control use of `posix_fallocate()` (Thomas Munro) [&sect;](https://postgr.es/c/33e3de6d7)
...
-  Honor `open()`'s `O_CLOEXEC` flag on Windows (Bryan Green, Thomas Munro) [&sect;](https://postgr.es/c/bebb281b0) [&sect;](https://postgr.es/c/a7d06e74d) [&sect;](https://postgr.es/c/4da5c33a3)
...
-  Fix failure to parse long options on the server command line in Solaris executables built with meson (Tom Lane) [&sect;](https://postgr.es/c/5eac1d68f)
-  Support process title changes on GNU/Hurd (Michael Banck) [&sect;](https://postgr.es/c/bcfca332f)
-  Fix psql's tab completion for `VACUUM` option values (Yugo Nagata) [&sect;](https://postgr.es/c/4e1376900)
-  In psql command prompts, do not show a value for `%P` (pipeline status) when there is no server connection (Chao Li) [&sect;](https://postgr.es/c/d42735b1e)

   This makes `%P` act like other prompt escape sequences whose values depend on the active connection.
-  Fix pg_dump's logic for collecting sequence values (Nathan Bossart) [&sect;](https://postgr.es/c/39d555576) [&sect;](https://postgr.es/c/56e1f5010)
...
-  Fix potentially-incorrect quoting of `oauth_validator_libraries` values by pg_dump (ChangAo Chen) [&sect;](https://postgr.es/c/61c78e1f4)

   pg_dump applied the wrong quoting rule if it needed to dump a value of this setting.
-  Avoid pg_dump assertion failure in binary-upgrade mode (Vignesh C) [&sect;](https://postgr.es/c/573e679a2)
...
-  Fix incorrect error handling in pgbench with multiple `\syncpipeline` commands in pipeline mode (Yugo Nagata) [&sect;](https://postgr.es/c/00e64e35c)
...
-  Make pg_resetwal print the updated value when changing OldestXID (Heikki Linnakangas) [&sect;](https://postgr.es/c/19594271c)

   It already did that for every other variable it can change.
-  Make pg_resetwal allow setting next multixact xid to 0 or next multixact offset to UINT32_MAX (Maxim Orlov) [&sect;](https://postgr.es/c/8747b969f)
...
-  In `contrib/amcheck`, use the correct snapshot for btree index parent checks (Mihail Nikalayeu) [&sect;](https://postgr.es/c/df93f94dd) [&sect;](https://postgr.es/c/3c83a2a0a)

   The previous coding caused spurious errors when examining indexes created with `CREATE INDEX CONCURRENTLY`.
-  Fix `contrib/amcheck` to handle “half-dead” btree index pages correctly (Heikki Linnakangas) [&sect;](https://postgr.es/c/19e786727)
...
-  Fix `contrib/amcheck` to handle incomplete btree root page splits correctly (Heikki Linnakangas) [&sect;](https://postgr.es/c/50c63ebb0)

   `amcheck` could report a false error about “block is not true root”.
-  Fix excessive memory allocation in `contrib/pg_buffercache` (David Geier) [&sect;](https://postgr.es/c/580b5c2f3)

   The code allocated twice as much memory as it needed for NUMA page status.
-  Fix edge-case integer overflow in `contrib/intarray`'s selectivity estimator for `@@` (Chao Li) [&sect;](https://postgr.es/c/07c1c6ec5)

   This could cause poor selectivity estimates to be produced for cases involving the maximum integer value.
-  Fix multibyte-encoding issue in `contrib/ltree` (Jeff Davis) [&sect;](https://postgr.es/c/f79e239e0)

   The previous coding could pass an incomplete multibyte character to `lower()`, probably resulting in incorrect behavior.
-  Avoid crash in `contrib/pg_stat_statements` when an `IN` list contains both constants and non-constant expressions (Sami Imseih) [&sect;](https://postgr.es/c/3304e97b1)
-  Update time zone data files to tzdata release 2025c (Tom Lane) [&sect;](https://postgr.es/c/6574bee64)
